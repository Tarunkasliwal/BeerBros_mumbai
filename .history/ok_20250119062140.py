...
class SearchAnalyzer:
    def __init__(self):
        self.serpapi_key = os.getenv('SERPAPI_API_KEY', '03c96cdc4bf9e2ecce3ae82c69d2414c7c432d625ce33f3ec2fcea745d772337')
    ...
    def fetch_data(self, query, limit=100):
        ...
        try:
            ...
            for result in news_results:
                ...
                source = str(result.get('source', '')) if result.get('source') else 'Unknown'
            ...
            results = self.get_sample_data(query, limit)
    ...
    def get_sample_data(self, query, limit):
        """Provide sample data when API fails"""
        sample_data = []
        sample_texts = [
            "Positive news about the latest developments",
            "Some concerns have been raised about recent events",
            "Neutral coverage of the ongoing situation",
            "Major breakthrough announced today",
            "Critics point out potential issues"
        ]
        
        for i in range(min(limit, len(sample_texts))):
            sentiment = self.analyze_sentiment(sample_texts[i])
            sample_data.append({
                'id': str(i),
                'title': f"Sample Article {i+1}",
                'text': sample_texts[i],
                'source': f"Source {i+1}",  # Ensure source is a string
                'date': datetime.now().strftime("%Y-%m-%d"),
                'link': "https://example.com",
                'sentiment_score': float(sentiment['compound']),  # Ensure score is float
                'sentiment_category': self.get_sentiment_category(sentiment['compound']),
                'analysis_timestamp': datetime.utcnow().isoformat()
            })
...
class AdGenerator:
    ...
    def generate_ad_ideas(self, data_df):
        ...
        if not trending:
            trending = ['trending', 'popular', 'innovative', 'exciting', 'new']
    ...
    if 'data' in st.session_state:
        ...
        with tab2:
            ...
            try:
                ...
                # Source sentiment analysis with error handling
                if not df.empty and 'source' in df.columns and 'sentiment_score' in df.columns:
                    # Convert source to string and group
                    df['source'] = df['source'].astype(str)
                    source_sentiment = df.groupby('source', as_index=False)['sentiment_score'].mean()
                    ...
                    if not source_sentiment.empty:
                        ...
                        st.warning("No source sentiment data available")
                else:
