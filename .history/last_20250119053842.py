...
class CreativeAssetGenerator:
    def __init__(self):
        self.color_schemes = {
            'positive': ['#4CAF50', '#81C784', '#C8E6C9'],
            'solution_oriented': ['#2196F3', '#64B5F6', '#BBDEFB'],
        }
...
class OllamaAdGenerator:
    def __init__(self, base_url="http://localhost:11434"):
        self.base_url = base_url
    
    def generate_ad_prompt(self, company_info, sentiment_data):
        avg_sentiment = sentiment_data['sentiment_score'].mean()
        sentiment_category = 'positive' if avg_sentiment > 0 else 'negative'
        
        if sentiment_category == 'positive':
            focus = "amplify positive sentiment and customer satisfaction"
        else:
            focus = "address concerns while highlighting improvements"
        
        prompt = f"""Create an engaging, humorous, and trend-driven advertisement for {company_info['name']}, 
        a {company_info['type']}. Current sentiment analysis shows {sentiment_category} feedback, 
        so let's {focus}.

        Target audience: {company_info['target_audience']}
        
        The ad should:
        - Include references to popular memes or viral moments
        - Use casual, relatable language that resonates with {company_info['target_audience']}
        - Highlight {company_info['key_points']}
        - Be optimized for {', '.join(company_info['platforms'])}
        
        Structure:
        1. Hook inspired by trending content
        2. Showcase the brand's {company_info['showcase_feature']} in a funny way
        3. Strong call to action
        4. Keep it short (15-30 seconds) and visually dynamic

        Current sentiment metrics:
        - Average sentiment: {avg_sentiment:.2f}
        - Key topics: {', '.join(sentiment_data['trending_topics'][:3])}
        """
        return prompt
    ...
    def generate_ad(self, prompt):
        try:
            ...
            print(f"Error generating ad with Ollama: {e}")
            return None

class EnhancedTwitterAnalyzer(TwitterAnalyzer):
    def __init__(self):
        super().__init__()
        self.ad_generator = AdGenerator()
        self.ollama_generator = OllamaAdGenerator()
    
    def get_sentiment_summary(self, df):
        return {
            'sentiment_score': df['sentiment_score'],
            'trending_topics': self.ad_generator.analyze_trending_topics(df),
            'key_benefits': self.ad_generator.extract_key_benefits(
                df[df['sentiment_category'] == 'positive']
            ),
            'pain_points': self.ad_generator.identify_pain_points(
...
def main():
    ...
    with st.sidebar:
        ...
        company_info = {
            ...
            'showcase_feature': st.text_input("Main Feature to Showcase", "Product Features")
        }
        ...
        if st.button("Analyze"):
            with st.spinner("Analyzing tweets and generating ads..."):
                df = analyzer.fetch_tweets(query, limit)
                st.session_state['data'] = df
                
                sentiment_summary = analyzer.get_sentiment_summary(df)
                
                prompt = analyzer.ollama_generator.generate_ad_prompt(
                    company_info,
                    sentiment_summary
                )
